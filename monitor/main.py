...
def main():
    ...
    print(f"[INFO] Model: {config.get('model_name', 'yolo11m.pt')}")
    print(f"[INFO] Total Zona Kursi: {len(chair_zones)}")
    print(f"[INFO] Ambang Batas Keyakinan: {config.get('confidence', 0.1)}")
    print(f"[INFO] Upper Body Ratio: {config.get('upper_body_ratio', 0.5)}")
    print(f"[INFO] Menghubungkan ke sumber video...")

    # Inisialisasi pendeteksi & logika aturan
    detector = PersonDetector(
        model_name=config.get("model_name", "yolo11m.pt"),
    ...
    try:
        while True:
            ...
            # Gunakan waktu simulasi video untuk file offline, atau waktu sistem untuk webcam/RTSP live
            is_live = str(source).isdigit() or str(source).startswith("rtsp://") or str(source).startswith("http://")
